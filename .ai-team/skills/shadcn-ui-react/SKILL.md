...
`src/App.css` - **CRITICAL: Must include CSS custom properties for shadcn/ui**:
```css
@tailwind base;
@tailwind components;
@tailwind utilities;
...
`tailwind.config.js` - **Must extend theme to map CSS variables to Tailwind classes**:
```javascript
export default {
  content: [
    "./index.html",
...
**Why CSS Variables Are Required:**
shadcn/ui components use semantic class names like `bg-background`, `border-input`, `text-muted-foreground` that reference CSS custom properties. Without defining these variables in App.css AND mapping them in tailwind.config.js, all component styling will be invalid and the UI will render completely unstyled. This is the #1 mistake when manually setting up shadcn/ui (the CLI tool handles this automatically).
...
- **Don't skip CSS variable definitions** — Without CSS custom properties in App.css, all components will be unstyled. This is the most common and silent failure mode.
- **Don't define CSS variables without Tailwind config mapping** — Both pieces are required: variables in CSS AND color mapping in tailwind.config.js.
