...
func (h *Handler) Handle(ctx context.Context, r slog.Record) error {
	...
	timeAttr := slog.Attr{
		Key:   slog.TimeKey,
		Value: slog.StringValue(r.Time.Format(time.RFC3339)),
	}
	if h.r != nil {
	...
	if h.outputEmptyAttrs || len(attrs) > 0 {
		...
		// This is setup so that when debug is enabled it does longer logging that in the end causes more lines in the journalctl for easier reading of errors
		if isDebug == "1" {
